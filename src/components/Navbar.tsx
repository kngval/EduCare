...
function Navbar() {
  ...
  return (
    <>
      <nav className={`${toggle == false ? "block" : "hidden"} navbar-main lg:hidden sm:px-12 sm:py-6`}>
        <div className={` flex justify-between  bg-customBlue2 rounded-lg py-3 px-8`}>
          ...
          <div onClick={() => setToggle(!toggle)} className={`${toggle == false ? "block" : "hidden"} w-8 cursor-pointer`}>
      ...
      <div className={`absolute min-h-screen top-0 -left-full lg:static ${toggle ? "left-0" : "-left-full"} transition-all ease-in-out duration-500 w-[250px] sm:w-[300px] pb-24  bg-customBlue2 `}>
